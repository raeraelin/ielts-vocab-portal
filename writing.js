...
function renderQuiz() {
  ...
  let displaySentence = buildParagraphContent(item.sentence);
  ...
  item.blanks.forEach(blankObj => {
    ...
    const regex = new RegExp(`\\b${escapeRegExp(blankObj.word)}\\b`, 'i');
  ...
  exerciseDiv.innerHTML = headerContent + `
    <div class="essay-content" style="line-height:2.5; font-size: 1.15em; color: #202124;">
      ${displaySentence}
    </div>
  ` + quoteContent;
...
function buildParagraphContent(sentenceData) {
  if (Array.isArray(sentenceData)) {
    return sentenceData
      .map((part, index) => formatParagraph(part, index === sentenceData.length - 1))
      .join("");
  }

  return formatParagraph(sentenceData, true);
}

function formatParagraph(part, isLastParagraph) {
  const text = String(part || "").trim();
  if (!text) return "";

  const paragraphStyle = isLastParagraph
    ? "margin: 0; text-indent: 2em;"
    : "margin: 0 0 26px 0; text-indent: 2em;";

  if (/^<p[\s>]/i.test(text)) {
    return applyParagraphStyle(text, paragraphStyle);
  }

  return `<p style="${paragraphStyle}">${text}</p>`;
}

function applyParagraphStyle(paragraphHtml, paragraphStyle) {
  if (/^<p[^>]*style=['"][^'"]*['"][^>]*>/i.test(paragraphHtml)) {
    return paragraphHtml.replace(
      /^<p([^>]*style=['"])([^'"]*)(['"][^>]*)>/i,
      `<p$1$2; ${paragraphStyle}$3>`
    );
  }

  return paragraphHtml.replace(/^<p([^>]*)>/i, `<p$1 style="${paragraphStyle}">`);
}
...
function createBulb(input) {
  ...
  ['拼字', '定義', '同義詞', '直接給答案'].forEach(type => {
    ...
    btn.onclick = () => {
      if (type === '拼字') {
        resText.innerText = correctAnswer[0] + ' _ '.repeat(correctAnswer.length - 1);
      }

      if (type === '定義') {
        resText.innerText = hints.definition;
      }

      if (type === '同義詞') {
        resText.innerText = hints.synonym;
      }
